...
void CPU::runWorkerLoop() {
    std::string schedulerAlgo = ConfigManager::getInstance()->getScheduler(); // e.g., "fcfs" or "rr"
    uint32_t timeQuantum = ConfigManager::getInstance()->getQuantumCycles();

    while (isRunning) {
        currentProcess = ProcessScheduler::getInstance()->fetchNextProcess();

        if (currentProcess != nullptr) {
            // 1. Transition state to RUNNING before execution
            currentProcess->setState(ProcessState::RUNNING);

            currentProcess->setCoreAssigned(this->coreID);
